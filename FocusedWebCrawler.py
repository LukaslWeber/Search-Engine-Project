...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", action="store_true", help="Start crawling with the predefined frontier)")
    parser.add_argument("-r", action="store_true", help="Continue crawling")

    # Parse the command-line arguments
    args = parser.parse_args()

    # Check which arguments were provided and call the corresponding functions
    if args.n:
        crawler = FocusedWebCrawler(frontier=urls, max_pages=10000)
    elif args.r:
        crawler = FocusedWebCrawler(frontier=None, max_pages=10000)
    else:
        print("No valid option provided. Use either -n or -r.")
        raise SystemExit
